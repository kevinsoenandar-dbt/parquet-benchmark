"""
Uploads the 4 dummy parquet files (generated by generate_dummy_parquet.py)
to a Snowflake internal stage, so both the existing Snowpark/pyarrow
approach and the audit_helper approach can read them from the same place.

Creates the stage `benchmark_stage` if it doesn't exist, then PUTs all
files under dummy_data/ into @benchmark_stage/benchmark/. AUTO_COMPRESS is
disabled since parquet is already compressed.
...
def main():
    ...
    if not files:
        sys.exit(f"No .parquet files found in {data_dir} — run generate_dummy_parquet.py first")
